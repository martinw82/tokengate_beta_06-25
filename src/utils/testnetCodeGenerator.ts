...
export const generateTestnetVerificationUrl = (config: TestnetTokenGateConfig): string => {
  const baseUrl = config.appBaseUrl.replace(/\/$/, '');

  const params = new URLSearchParams();
  params.append('network', config.network);
  params.append('environment', 'testnet');
  ...
  return `${baseUrl}/testnet/gate?${params.toString()}`;
};

export const generateTestnetEmbedCode = (config: TestnetTokenGateConfig): string => {
  const baseUrl = config.appBaseUrl.replace(/\/$/, '');
  let content = '';
  ...
  if (config.network === 'algorand') {
    walletProviderCode = `
        ...
        try {
          ...
          if (!accounts || accounts.length === 0) {
            throw new Error('No Algorand testnet accounts found');
          }
    ...
    const networkMappings = {
      ...
      polygon: { chainId: '0x13881' }, // Mumbai
      base: { chainId: '0x14a33' }, // Base Goerli
    };
    ...
    verificationCode = `
        ...
        const response = await fetch(config.apiUrl, {
          ...
          body: JSON.stringify({
            ...
            ${config.tokenId ? `tokenId: config.tokenId,` : ''}
            ${config.network === 'any-evm' ? `rpcUrl: "YOUR_TESTNET_RPC_URL_HERE", // Replace with your RPC URL` : ''}
          }),
        });`;
  }

  return `<!-- TokenGate Testnet Embed Code -->
<div id="token-gate-container">
  ...
  <div id="token-gate-connect" style="text-align: center; padding: 20px;">
    <p>This content requires testnet token ownership to view.</p>
  ...
  <div id="token-gate-error" style="display: none; text-align: center; padding: 20px; color: #ef4444;">
    Access denied. You don't have the required testnet token.
  </div>
</div>
...
<script>
  (function() {
    // TokenGate Testnet Configuration
    const config = {
      network: "${config.network}",
      environment: "testnet",
      tokenAddress: "${config.tokenAddress}",
      ...
      apiUrl: "${baseUrl}/testnet/api/check-token"
    };
...
export const generateTestnetWordPressCode = (config: TestnetTokenGateConfig): string => {
  ...
  const customAttributes = config.network === 'any-evm' ? ` custom_rpc="YOUR_TESTNET_RPC_URL_HERE"` : '';

  return `[tokengate network="${config.network}" environment="testnet" token_address="${config.tokenAddress}" token_type="${config.tokenType}"${tokenAttributes}${actionAttributes}${customAttributes}]
  Your protected testnet content goes here. This will only be visible to users who own the required testnet token.
[/tokengate]`;
};

// Helper function to generate access denied HTML
const generateTestnetAccessDeniedHTML = (): string => {
  return `<!DOCTYPE html>
      <html>
      <head>
        <title>Testnet Access Denied</title>
        <style>
          body {
      ...
      <body>
        <div class="container">
          <h1>Testnet Access Denied</h1>
          <p>You don't have the required testnet token to access this content.</p>
          <p>The requested content requires ownership of a specific token on a testnet blockchain network.</p>
...
export const generateTestnetServerCode = (config: TestnetTokenGateConfig): string => {
  const baseUrl = config.appBaseUrl.replace(/\/$/, '');
  ...
  if (config.network === 'algorand') {
    verificationCode = `  // Verify token ownership on Algorand Testnet
const verifyTokenOwnership = async (userAddress, config) => {
  ...
  try {
    ...
    console.error('Error verifying Algorand testnet token ownership:', error);
    throw error;
  }
};`;
  } else if (config.network === 'any-evm') {
    verificationCode = `  // Verify token ownership on custom EVM testnet chain
const verifyTokenOwnership = async (userAddress, config) => {
  // For custom EVM chains, we need an RPC URL
  if (!config.rpcUrl) {
    throw new Error('RPC URL is required for custom EVM testnet chains');
  }
  ...
  try {
    ...
    console.error('Error verifying testnet token ownership:', error);
    throw error;
  }
};`;
  } else {
    // Default EVM verification
    verificationCode = `  // Verify testnet token ownership
...
// const algosdk = require('algosdk');`;
  ...
  if (config.network === 'any-evm') {
    ...
    // Required for custom EVM testnet chains
    rpcUrl: "https://your-custom-testnet-chain-rpc.url",`;
  }

  // Network-specific testnet chain IDs
  let testnetChainInfo = '';
  if (config.network === 'ethereum') {
    testnetChainInfo = `// Using Goerli testnet (chainId: 0x5)`;
  } else if (config.network === 'polygon') {
    testnetChainInfo = `// Using Mumbai testnet (chainId: 0x13881)`;
  } else if (config.network === 'base') {
    testnetChainInfo = `// Using Base Goerli testnet (chainId: 0x14a33)`;
  }

  return `// TokenGate Testnet Server Middleware (Express.js)
// ----------------------------------------
// Save this as 'tokengate-testnet-middleware.js'
...
// Create TokenGate Testnet middleware
const createTestnetTokenGate = (app, options) => {
  // Default options
  const defaultOptions = {
    // TokenGate testnet configuration
    network: "${config.network}", ${testnetChainInfo}
    tokenAddress: "${config.tokenAddress}",
    tokenType: "${config.tokenType}",
    ${config.tokenType === 'ERC-20' || config.tokenType === 'ASA' ? `minBalance: "${config.minBalance}",` : ''}
    ...
    action: {
      type: "${config.action.type}",
      ${config.action.type === 'redirect' ? `redirectUrl: "${config.action.redirectUrl || '/testnet-access-denied'}"` : ''}
      ${config.action.type === 'message' ? `message: "${config.action.message || 'Testnet access granted!'}"` : ''}
    },
    ...
    // Routes to protect
    protectedRoutes: ['/testnet-protected', '/testnet-members/*'],
    
    // Session configuration
    cookieName: 'tokengate_testnet_session',
  ...
  const tokenGateMiddleware = async (req, res, next) => {
    ...
    try {
      ...
      console.error('Testnet TokenGate verification error:', error);
      return handleUnauthorized(req, res, config);
    }
  };
  ...
  app.get('/testnet-access-denied', (req, res) => {
    res.status(403).send(generateTestnetAccessDeniedHTML());
  });
...
const handleUnauthorized = (req, res, config) => {
  ...
  if (config.action.type === 'redirect') {
    const redirectUrl = config.action.redirectUrl || '/testnet-access-denied';
    return res.redirect(redirectUrl);
  } else {
    return res.redirect('/testnet-access-denied');
  }
};

module.exports = { createTestnetTokenGate };

// USAGE EXAMPLE:
// ---------------------------------------------
// const express = require('express');
// const { createTestnetTokenGate } = require('./tokengate-testnet-middleware');
// 
// const app = express();
// 
// // Set up TokenGate testnet middleware
// createTestnetTokenGate(app, {
//   // Override any default options if needed
//   protectedRoutes: ['/testnet-members', '/testnet-premium/*']${config.network === 'any-evm' ? `,
//   rpcUrl: "https://your-custom-testnet-chain-rpc.url"` : ''}
// });
// 
// // Define your routes
...
// });
// 
// app.get('/testnet-protected', (req, res) => {
//   // This route is protected by TokenGate
//   res.send('You have access to testnet protected content!');
// });
// 
// app.listen(3000, () => {
