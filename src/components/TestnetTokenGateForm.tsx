...
import { useTestnetTokenGate } from '../hooks/useTestnetTokenGate';
import { TestnetTokenGateConfig, Network, TokenType } from '../types';
import { Lock, Code, Eye, RefreshCw } from 'lucide-react';

export const TestnetTokenGateForm: React.FC = () => {
  const { generateCode, isGenerating } = useTestnetTokenGate();
  const [activeTab, setActiveTab] = useState<'configure' | 'preview'>('configure');
  const [config, setConfig] = useState<TestnetTokenGateConfig>({
    network: 'base',
    tokenAddress: '',
    tokenType: 'ERC-20',
  ...
  // Network mapping for testnet explanations
  const getTestnetExplanation = (network: Network): string => {
    const networkMap: {[key: string]: string} = {
      'ethereum': 'Goerli/Sepolia',
      'base': 'Base Goerli/Sepolia',
      'polygon': 'Mumbai',
      'algorand': 'Algorand Testnet',
      'any-evm': 'Custom Testnet'
    };
          ...
          <div className="p-6">
            <div className="space-y-6">
              <div className="space-y-4">
                <h2 className="text-lg font-semibold text-white">Token Gate Configuration (Testnet)</h2>
                
                <div className="p-4 bg-blue-900/20 border border-blue-800 rounded-lg mb-4">
                  <p className="text-blue-300 text-sm">
                    You are configuring a token gate for <strong>testnet</strong> networks. 
                    This page will use {getTestnetExplanation(config.network)} for verification.
                  </p>
                </div>
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-300 mb-1">
                    {config.network === 'algorand' ? 'Testnet Asset ID' : 'Testnet Token Contract Address'}
                  </label>
                  <input
                    type="text"
                    ...
                    placeholder={config.network === 'algorand' ? "Testnet Asset ID (e.g., 123456789)" : "0x..."}
                    value={config.tokenAddress}
                    onChange={(e) => handleChange('tokenAddress', e.target.value)}
                  />
                  {config.network !== 'algorand' && (
                    <p className="mt-1 text-xs text-blue-300">
                      Using {getTestnetExplanation(config.network)} testnet.
                    </p>
                  )}
                  {config.network === 'any-evm' && (
                    <p className="mt-1 text-xs text-gray-400">
                      You'll need to specify the testnet RPC URL in your implementation.
                    </p>
                  )}
                </div>
